import os, csv, argparse, importlib, inspect
import numpy as np
import torch
...
SEEDS     = [0, 1, 2, 3, 4]
T_FIXED   = 250            # same fixed timestep used in training-file extract_embeddings


# ── load a diffusion variant by filename (without .py) ──
def load_model(model_name):
    mod = importlib.import_module(model_name)
    # find the UNet class (name starts with DiffusionUNet)
    unet_cls = None
    for n, obj in inspect.getmembers(mod, inspect.isclass):
        if n.startswith("DiffusionUNet"):
            unet_cls = obj
            break
    if unet_cls is None:
        raise RuntimeError(f"No DiffusionUNet* class found in {model_name}")
    model = unet_cls().to(DEVICE)
    state = torch.load(mod.BEST_PATH, map_location=DEVICE, weights_only=False)
    model.load_state_dict(state)
    model.eval()
    scheduler = mod.NoiseScheduler(T=mod.T_MAX, device=DEVICE)
    return model, scheduler


# ── embeddings: add noise at T_FIXED, encode, mean-pool ──
@torch.no_grad()
def get_embeddings(model, scheduler):
    ...
    for imgs, y, _ in dl:
        imgs = imgs.to(DEVICE)
        B = imgs.shape[0]
        t = torch.full((B,), T_FIXED, device=DEVICE, dtype=torch.long)
        xt, _ = scheduler.add_noise(imgs, t)
        z = model.encode(xt, t)
...
def main():
    ...
    ap.add_argument("--model", required=True, help="Diffusion filename without .py")
    args = ap.parse_args()
    name = args.model
    ...
    model, scheduler = load_model(name)

    print(f"[{name}] extracting embeddings (t={T_FIXED})...", flush=True)
    embs, labels, full = get_embeddings(model, scheduler)
    ...
    write_txt(os.path.join(out_dir, "linear_probe", "results.txt"),
              {"auc_mean": probe["lin_auc_mean"], "auc_std": probe["lin_auc_std"],
               "acc_mean": probe["lin_acc_mean"], "seeds": SEEDS, "t_fixed": T_FIXED,
