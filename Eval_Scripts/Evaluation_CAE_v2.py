...
SEEDS    = [0, 1, 2, 3, 4]      # for probe variance


# ── load a CAE variant by filename (without .py) ──
def load_model(model_name):
    mod = importlib.import_module(model_name)
    model = mod.CAE().to(DEVICE)
...
# ── embeddings for the full ARMD-task set + labels + co-label group ──
@torch.no_grad()
...
# ── silhouette on binary ARMD vs rest (full set) ──
...
def main():
    ...
    ap.add_argument("--model", required=True, help="CAE filename without .py (e.g. CAE_v4)")
    args = ap.parse_args()
    name = args.model
