...
CKPT_DIR   = "checkpoints/2earlyskips"
CKPT_PATH  = os.path.join(CKPT_DIR, "last.pt")
BEST_PATH  = os.path.join(CKPT_DIR, "best_CAE_2earlyskips.pt")
LR         = 1e-3
BATCH_SIZE = 64
EPOCHS     = 50
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        s1 = self.block1(x)   # 64  x 256x256  ← skip
        s2 = self.block2(s1)  # 128 x 128x128  ← skip
        x  = self.block3(s2)  # 256 x 64x64
        x  = self.block4(x)   # 512 x 32x32
        x  = self.block5(x)   # 512 x 16x16
        z  = self.block6(x)   # 512 x 8x8
        return z, (s1, s2)
...
class Decoder(nn.Module):
    ...
    def __init__(self):
        ...
        # Block 1: 512x8x8 -> 512x16x16 — no skip
        self.block1 = nn.Sequential(
        ...
        # Block 2: 512x16x16 -> 512x32x32 — no skip
        self.block2 = nn.Sequential(
            nn.ConvTranspose2d(512, 512, kernel_size=3, stride=2, padding=1, output_padding=1),
            nn.BatchNorm2d(512),
            nn.LeakyReLU(0.1, inplace=True),
        ...
        # Block 3: 512x32x32 -> 256x64x64 — no skip
        self.block3 = nn.Sequential(
    ...
    def forward(self, z, skips):
        ...
        x = self.block1(z)                          # 512 x 16x16 — no skip
        x = self.block2(x)                          # 512 x 32x32 — no skip
        x = self.block3(x)                          # 256 x 64x64 — no skip
