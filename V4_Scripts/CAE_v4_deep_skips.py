...
CKPT_DIR   = "checkpoints/2deepskips"
CKPT_PATH  = os.path.join(CKPT_DIR, "last.pt")
BEST_PATH  = os.path.join(CKPT_DIR, "best_CAE_2deepskips.pt")
LR         = 1e-3
BATCH_SIZE = 64
EPOCHS     = 50
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        x  = self.block1(x)   # 64  x 256x256
        x  = self.block2(x)   # 128 x 128x128
        x  = self.block3(x)   # 256 x 64x64
        s4 = self.block4(x)   # 512 x 32x32  ← skip
        s5 = self.block5(s4)  # 512 x 16x16  ← skip
...
class Decoder(nn.Module):
    ...
    def __init__(self):
        ...
        # Block 4: 256x64x64 -> 128x128x128 — no skip
        self.block4 = nn.Sequential(
        ...
        # Block 5: 128x128x128 -> 64x256x256 — no skip
        self.block5 = nn.Sequential(
    ...
    def forward(self, z, skips):
        ...
        x = self.block4(x)                          # 128 x 128x128 — no skip
        x = self.block5(x)                          # 64  x 256x256 — no skip
